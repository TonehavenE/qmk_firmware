...
enum combo_events {
    /* Steno-lite combos */
    // These let me type common words in a single stroke.
    ...
    // My optimal mouse combos
    RS_MOUSE,
    ST_MOUSE,
    RT_MOUSE,
};

const uint16_t PROGMEM AboutCombo[] = {KC_A, KC_B, COMBO_END};
const uint16_t PROGMEM ActuallyCombo[] = {KC_A, KC_C, COMBO_END }; 
const uint16_t PROGMEM AndCombo[] = {KC_A, KC_N, COMBO_END };
const uint16_t PROGMEM BecauseCombo[] = {KC_B, KC_C, COMBO_END};
const uint16_t PROGMEM ComeCombo[] = {KC_C, KC_O, COMBO_END};
const uint16_t PROGMEM DefinitelyCombo[] = {KC_D, KC_I, COMBO_END};
const uint16_t PROGMEM EgCombo[] = {KC_E, KC_G, COMBO_END};
const uint16_t PROGMEM FinallyCombo[] = {KC_F, KC_I, COMBO_END};
const uint16_t PROGMEM GiveCombo[] = {KC_G, KC_I, COMBO_END };
const uint16_t PROGMEM GoodCombo[] = {KC_G, KC_O, COMBO_END };
const uint16_t PROGMEM HaveCombo[] = {KC_H, KC_V, COMBO_END};
const uint16_t PROGMEM IeCombo[] = {KC_I, KC_E, COMBO_END};
const uint16_t PROGMEM TionCombo[] = {KC_O, KC_I, COMBO_END};
const uint16_t PROGMEM WouldCombo[] = {KC_W, KC_D, COMBO_END};
const uint16_t PROGMEM MakeCombo[] = {KC_M, KC_K, COMBO_END};
const uint16_t PROGMEM MentCombo[] = {KC_M, KC_T, COMBO_END};
const uint16_t PROGMEM TheCombo[] = {KC_T, KC_H, COMBO_END};
const uint16_t PROGMEM ThisCombo[] = {KC_T, KC_I, COMBO_END};
const uint16_t PROGMEM UntilCombo[] = {KC_U, KC_N, COMBO_END};
const uint16_t PROGMEM YoureCombo[] = {KC_U, KC_R, COMBO_END};
const uint16_t PROGMEM WhatCombo[] = {KC_W, KC_A, COMBO_END};
const uint16_t PROGMEM WhichCombo[] = {KC_W, KC_I, COMBO_END};
const uint16_t PROGMEM YourCombo[] = {KC_Y, KC_R, COMBO_END};
const uint16_t PROGMEM YouCombo[] = {KC_Y, KC_U, COMBO_END};
const uint16_t PROGMEM OtherCombo[] = {KC_O, KC_T, COMBO_END};
const uint16_t PROGMEM UnfortunatelyCombo[] = {KC_U, KC_F, COMBO_END};
const uint16_t PROGMEM CantCombo[] = {KC_QUOTE, KC_C, COMBO_END};
const uint16_t PROGMEM DontCombo[] = {KC_QUOTE, KC_D, COMBO_END};
const uint16_t PROGMEM HasntCombo[] = {KC_QUOTE, KC_H, COMBO_END};
const uint16_t PROGMEM ArentCombo[] = {KC_QUOTE, KC_R, COMBO_END};
const uint16_t PROGMEM IsntCombo[] = {KC_QUOTE, KC_S, COMBO_END};
const uint16_t PROGMEM ItsCombo[] = {KC_QUOTE, KC_T, COMBO_END};
const uint16_t PROGMEM WasntCombo[] = {KC_QUOTE, KC_W, COMBO_END};

// Programming Combos
const uint16_t PROGMEM ATHomeCombo[]      = {KC_A,     KC_T,    COMBO_END};
const uint16_t PROGMEM NOEndCombo[]       = {KC_N,     KC_O, COMBO_END};
...
combo_t key_combos[COMBO_COUNT] = {
    [AB_About] = COMBO_ACTION(AboutCombo),
    [AC_Actually] = COMBO_ACTION(ActuallyCombo),
    [AN_And] = COMBO_ACTION(AndCombo),
    [BC_Because] = COMBO_ACTION(BecauseCombo),
    [CO_Come] = COMBO_ACTION(ComeCombo),
    [DI_Definitely] = COMBO_ACTION(DefinitelyCombo),
    [EG_Eg] = COMBO_ACTION(EgCombo),
    [FI_Finally] = COMBO_ACTION(FinallyCombo),
    [GI_Give] = COMBO_ACTION(GiveCombo),
    [GO_Good] = COMBO_ACTION(GoodCombo),
    [HV_Have] = COMBO_ACTION(HaveCombo),
    [IE_Ie] = COMBO_ACTION(IeCombo),
    [IO_Tion] = COMBO_ACTION(TionCombo),
    [WD_Would] = COMBO_ACTION(WouldCombo),
    [MK_Make] = COMBO_ACTION(MakeCombo),
    [MT_Ment] = COMBO_ACTION(MentCombo),
    [TH_The] = COMBO_ACTION(TheCombo),
    [TI_This] = COMBO_ACTION(ThisCombo),
    [UN_Until] = COMBO_ACTION(UntilCombo),
    [UR_Youre] = COMBO_ACTION(YoureCombo),
    [WA_What] = COMBO_ACTION(WhatCombo),
    [WI_Which] = COMBO_ACTION(WhichCombo),
    [YR_Your] = COMBO_ACTION(YourCombo),
    [YU_You] = COMBO_ACTION(YouCombo),
    [OT_Other] = COMBO_ACTION(OtherCombo),
    [UF_Unfortunately] = COMBO_ACTION(UnfortunatelyCombo),
    [QuoteC_Cant] = COMBO_ACTION(CantCombo),
    [QuoteD_Dont] = COMBO_ACTION(DontCombo),
    [QuoteH_Hasnt] = COMBO_ACTION(HasntCombo),
    [QuoteR_Arent] = COMBO_ACTION(ArentCombo),
    [QuoteS_Isnt] = COMBO_ACTION(IsntCombo),
    [QuoteT_Its] = COMBO_ACTION(ItsCombo),
    [QuoteW_Wasnt] = COMBO_ACTION(WasntCombo),

    [AT_HOME]      = COMBO(ATHomeCombo, KC_HOME),
    [NO_END]       = COMBO(NOEndCombo, KC_END),
...
void process_combo_event(uint16_t combo_index, bool pressed) {
    ...
    switch(combo_index) {
      ...
      case AC_Actually:
        if (pressed) {
        SEND_CAP_STRING("actually ", "Actually ");
      }
      break;
      ...
      case CO_Come:
        if (pressed) {
        SEND_CAP_STRING("come ", "Come ");
      }
      break;

      case DI_Definitely:
        if (pressed) {
        SEND_CAP_STRING("definitely ", "Definitely ");
      }
      break;

      case EG_Eg:
        if (pressed) {
        SEND_CAP_STRING("e.g., ", "E.g., ");
      }
      break;

      case FI_Finally:
        if (pressed) {
        SEND_CAP_STRING("finally ", "Finally ");
      }
      break;

      case GI_Give:
        if (pressed) {
        SEND_CAP_STRING("give ", "Give ");
      }
      break;

      case GO_Good:
        if (pressed) {
        SEND_CAP_STRING("good ", "Good ");
      }
      break;
      ...
      case IE_Ie:
        if (pressed) {
        SEND_CAP_STRING("i.e., ", "I.e., ");
      }
      break;

      case IO_Tion:
        if (pressed) {
        SEND_CAP_STRING("tion ", "Tion ");
      }
      break;
      ...
      break;
       ...
       case OT_Other:
        if (pressed) {
        SEND_CAP_STRING("other ", "Other ");
      }
      break;

       case UF_Unfortunately:
        if (pressed) {
        SEND_CAP_STRING("unfortunately ", "Unfortunately ");
      }
      break;
...
KC_LALT  , KC_Q ,   KC_W , KC_F    , LSFT_T(KC_P) , KC_G   ,               KC_J    ,    LSFT_T(KC_L), KC_U      , KC_Y      , KC_QUOTE    , KC_SCOLON    ,
